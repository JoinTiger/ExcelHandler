...
//    String getBatchId();
    ...
    @Query(value = "select distinct batch_id as batchId, mac_sn as macSn from product_info" + " where"
            ...
            + " if(ISNULL(?2) || length(trim(?4)) < 1, 1 = 1, mac_sn = ?4) and"
            + " if(ISNULL(?2) || length(trim(?5)) < 1, 1 = 1, batch_id = ?5) and"

            + " if(ISNULL(?2) || length(trim(?6)) < 1, 1 = 1, time >= ?6) and"
            + " if(ISNULL(?2) || length(trim(?7)) < 1, 1 = 1, time <= ?7)"
            ,nativeQuery = true)
